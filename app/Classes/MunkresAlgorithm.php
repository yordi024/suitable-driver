...
//cost matrix
    ...
    public function initData($aCostMatrix): void
    {
    ...
    private function showCostMatrix(): void
    {
    ...
    private function showRowCover(): void
    {
        $str = "row cover:";
        for($r = 0; $r < $this->nrow; $r++){
            $str .= $this->rowCover[$r] . ",";
    ...
    private function showMaskMatrix(): void
    {
        for($r = 0; $r < $this->nrow; $r++){
            $str = "";
            for($c = 0; $c < $this->ncol; $c++){
    ...
    private function showColCover(): void
    {
        $str = "col cover:";
        for($c = 0; $c < $this->ncol; $c++){
            $str .= $this->colCover[$c] . ",";
    ...
    public function runMunkres(): array
    {
        $done = false;
        while(!$done){
            if($this->debug){
    ...
    private function step_one(): void
    {
        $min_in_row = 10000;
        for($r = 0; $r < $this->nrow; $r++){
    ...
    private function step_two(): void
    {
        for($r = 0; $r < $this->nrow; $r++){
            for($c = 0; $c < $this->ncol; $c++){
                if($this->C[$r][$c] == 0 &&
    ...
    private function step_three(): void
    {
        $colcount = 0;
        for($r = 0; $r < $this->nrow; $r++){
            for($c = 0; $c < $this->ncol; $c++){
    ...
    private function find_a_zero(&$row, &$col): void
    {
        $r = 0;
        $c = 0;
        $done = false;
    ...
    private function star_in_row($row): bool
    {
        $tmp = false;
        for($c = 0; $c < $this->ncol; $c++){
            if($this->M[$row][$c] == 1){
    ...
    private function find_star_in_row($row): int
    {
        $col = -1;
        for($c = 0; $c < $this->ncol; $c++){
            if($this->M[$row][$c] == 1){
    ...
    private function step_four(): void
    {
        $row = -1;
        $col = -1;
        $done = false;
    ...
    private function find_star_in_col($c): int
    {
        $r = -1;
    ...
    private function find_prime_in_row($r): int
    {
        $c = -1;
    ...
    private function augment_path(): void
    {
    ...
    private function clear_covers(): void
    {
        for($r = 0; $r < $this->nrow; $r++){
            $this->rowCover[$r] = 0;
        }
    ...
    private function erase_primes(): void
    {
        for($r = 0; $r < $this->nrow; $r++){
            for($c = 0; $c < $this->ncol; $c++){
                if($this->M[$r][$c] == 2){
    ...
    private function step_five(): void
    {
    ...
    private function find_smallest(&$minval): void
    {
        for($r = 0; $r < $this->nrow; $r++){
            for($c = 0; $c < $this->ncol; $c++){
    ...
    private function step_six(): void
    {
        $minval = 1000000;
    ...
    private function step_seven(): void
    {
        if($this->debug){
            echo "\n\n-------Run Complete--------";
        }
